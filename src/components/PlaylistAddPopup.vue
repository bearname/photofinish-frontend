...
export default {
  ...
  methods: {
    ...
    async createPlaylist() {
      ...
      await this.createPlayList({name: this.name, privacy: this.privacy, events: videos});
    ...
    async saveToPlaylist(playlistId) {
      const videos = this.getCurrentVideos();
      await this.doPlaylistModification({action: playlistModification.AddVideo, playlistId: playlistId, videos: events})
      this.errors = this.getErrors();
